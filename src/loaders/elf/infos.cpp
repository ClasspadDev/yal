  ...
  for (const auto &note : *notes)
    if (std::strncmp(note.getName().data(), "NAME", note.getName().size()) ==
      ...
      name[note.getDesc().size()] =
          '\0'; // just in case it's not already (which is allowed)
      return name;
    }
...
std::unique_ptr<char[]> ELFLoader::getAuthor() {
  ...
  for (const auto &note : *notes)
    if (std::strncmp(note.getName().data(), "AUTHOR", note.getName().size()) ==
        0) {
...
std::unique_ptr<char[]> ELFLoader::getVersion() {
  ...
  for (const auto &note : *notes)
    if (std::strncmp(note.getName().data(), "VERSION", note.getName().size()) ==
        0) {
      auto version = std::make_unique<char[]>(note.getDesc().size() + 1);
      std::copy_n(note.getDesc().data(), note.getDesc().size(), version.get());
